...
const _ = require(`lodash`); // read files as `binary` from file system
...
async function onCreateNode({
  ...
  const content = await _loadNodeContent(node, loadNodeContent); // accept *all* options to pass to the sheet_to_json function

  const xlsxOptions = options; // alias legacy `rawOutput` to correct `raw` attribute if raw isn't already defined
  ...
  delete xlsxOptions.plugins; // Parse
  ...
  wb.SheetNames.forEach((n, idx) => {
    ...
    if (_.isArray(parsedContent)) {
      const csvArray = parsedContent.map((obj, i) => {
        ...
        return { ...obj,
          id: obj.id ? obj.id : createNodeId(`${node.id} [${n} ${i}] >>> ${node.extension}`),
          children: [],
          parent: node.id,
          internal: {
            contentDigest,
